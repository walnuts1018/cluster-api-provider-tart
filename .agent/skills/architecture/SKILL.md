...
多様な電源・ブート方式、Pull型Provisioning Agent、A/B OSスロット、CAPI Runtime SDKを用いたインプレース更新への移行計画は、[`docs/redesign/README.md`](../../../docs/redesign/README.md)を正本とする。

再設計に関する回答・実装・レビューを開始する前に、[`docs/redesign/conventions.md`](../../../docs/redesign/conventions.md)、[`docs/redesign/target-state.md`](../../../docs/redesign/target-state.md)、[`docs/redesign/architecture.md`](../../../docs/redesign/architecture.md)をこの順で読むこと。用語を独自に解釈せず、`conventions.md`の定義と要求レベルを使用すること。

以下の内容は現在の実装を説明する。再設計タスクでは、現在の`TartMachine.image/initrd/kernelParams`やBootstrap token消費を完成条件として新機能を追加せず、再設計文書のADRとタスク受け入れ条件に従うこと。実装の移行が完了した項目から、この節より下の現行設計を更新する。
