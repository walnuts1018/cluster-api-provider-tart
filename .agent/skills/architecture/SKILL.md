...
1. **Infrastructure Controller (The Brain)**: 単一のPod (`hostNetwork: true`) で稼働し、以下のサブシステムをGoroutineとして並行起動する。
   ...
   - **Embedded DHCP Server**: `insomniacslk/dhcp` を利用しProxyDHCPとして実装。既存ネットワークへの影響を防ぎつつPXEブート環境を提供。IPアドレスは配布せず、ブートファイル名のみを提供。
   - **Embedded TFTP Server**: `pin/tftp` を利用しiPXEバイナリ (`ipxe.efi`) を配信。
   ...
   ### ネットワークブートシーケンス
   1. **ProxyDHCP**: 物理PCがDHCP Discoverを送信 → 組み込みDHCPサーバーがiPXEブートローダのパスを応答（IPアドレスは配布しない）
   2. **TFTP**: 物理PCがiPXEブートローダをTFTPで取得 → 組み込みTFTPサーバーが`ipxe.efi`を配信。このバイナリはOCI Artifactとして配布され、Kubernetesの Image Volume Mount 機能を用いてTFTPルートにマウントされる。
