...
- [Infrastructure Cluster Contract](https://cluster-api.sigs.k8s.io/developer/providers/contracts/infra-cluster): 
  - `Spec.ControlPlaneEndpoint`: APIサーバーへの接続先を報告。
  - `Status.Initialization.Provisioned`: インフラ準備完了を示す。
  - `Paused` アノテーションや `managed-by` ラベルへの対応が必須。
- [Infrastructure Machine Contract](https://cluster-api.sigs.k8s.io/developer/providers/contracts/infra-machine):
  - `Spec.ProviderID`: クラウド上のインスタンスを一意識別するID。Nodeと一致させる。
  - `Status.Initialization.Provisioned`: VMがプロビジョニングされたことを示す。
  - プロビジョニング前に `spec.bootstrap.dataSecretName` の存在確認が必要。
