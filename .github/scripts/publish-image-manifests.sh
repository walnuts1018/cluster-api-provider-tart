#!/usr/bin/env bash
# arch別jobがpushしたdigest-onlyイメージを、target(image)ごとに1つのmanifest listへ束ねてpushする。
# 各archのdigest JSON群を突き合わせ、target集合・digest形式の整合性を検証してから
# docker buildx imagetools createでmulti-arch manifestを作成する。
set -euo pipefail
shopt -s nullglob
