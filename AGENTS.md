...
- 再設計タスクの検証は、可能な限りCIで再現できる形へ寄せてください。詳細は `docs/development/redesign/ci-verification.md` を正本とし、ローカル手順だけを完了証跡にしないでください。
...
- 実装を行う際には、docs/development/redesign/README.mdを確認し、adrやアーキテクチャなどのドキュメントを必要に応じて参照すること。
...
- 次期アーキテクチャへの移行計画、ADR、タスク分割は、`docs/development/redesign/README.md`を参照してください。再設計作業では最初に`docs/development/redesign/conventions.md`を読み、定義済み用語と要求レベルを使用してください。移行完了までは現行実装と目標設計を混同しないでください。
