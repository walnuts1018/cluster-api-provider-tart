...
- 現状まだ開発中で未リリースなので、破壊的変更はどのようなものでも自由に行ってください。変更を最小にすることよりも、実装の美しさや使いやすさなどを優先してください。v1alpha1については廃止予定なので、互換性を保ったり考慮したりする必要はありません。
- タスクを終える際には、次に行うべきタスクと、そのタスクを行うために利用するべきAI ModelとEffortを出力してください。AI Modelは、`gpt-5.4-mini`、`gpt-5.4`、`gpt-5.5`のいずれかを選択してください。Effortは、`low`、`medium`、`high`のいずれかを選択してください。
...
- 実装を行う際には、docs/redesign/README.mdを確認し、adrやアーキテクチャなどのドキュメントを必要に応じて参照すること。
...
- 次期アーキテクチャへの移行計画、ADR、タスク分割は、`docs/redesign/README.md`を参照してください。再設計作業では最初に`docs/redesign/conventions.md`を読み、定義済み用語と要求レベルを使用してください。移行完了までは現行実装と目標設計を混同しないでください。
