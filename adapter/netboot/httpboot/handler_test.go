...
func TestHandlerIPXEScript(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"discovery": {
			...
			wantBody:       "#!ipxe\necho Booting Talos (v1.14.0, discovery-schematic, source=discovery)\nchain https://factory.test.walnuts.dev/pxe/discovery-schematic/v1.14.0/metal-arm64\n",
		},
		"resolved": {
			...
			resolvedFound:  true,
			wantStatus:     http.StatusOK,
			wantBody:       "#!ipxe\necho Booting Talos (v1.15.0, resolved-schematic, source=resolved)\nchain https://factory.test.walnuts.dev/pxe/resolved-schematic/v1.15.0/metal-amd64\n",
			wantMAC:        requestMAC,
			wantCallCount:  1,
		},
		"not found falls back": {
			...
			discoveryImage: domainnetboot.DiscoveryImage{Version: discoveryVer, SchematicID: discoveryID},
			wantStatus:     http.StatusOK,
			wantBody:       "#!ipxe\necho Booting Talos (v1.14.0, discovery-schematic, source=discovery)\nchain https://factory.test.walnuts.dev/pxe/discovery-schematic/v1.14.0/metal-amd64\n",
			wantMAC:        requestMAC,
			wantCallCount:  1,
		},
