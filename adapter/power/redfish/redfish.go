// Package redfishはRedfish電源backendの実装を提供する。adapter/powerのFactoryから呼び出される。
package redfish
...
// BackendはRedfish Service RootとComputerSystem Reset actionだけを使う電源backendである。OS停止はTalosへ委譲し、PowerOffは明示的なGracefulShutdown actionとしてのみ提供する。
type Backend struct {
	baseURL  *url.URL
	systemID string
	username string
...
// NewはHTTPS endpoint、credential、TLS設定を検証してRedfish backendを構築する。
func New(config Config) (*Backend, error) {
	return newBackend(config, nil)
}
...
func newRedfishHTTPClient(config Config) (*http.Client, error) {
...
func (r *Backend) PowerOn(ctx context.Context) error {
...
func (r *Backend) PowerOff(ctx context.Context) error {
	systemURL, system, err := r.system(ctx)
	if err != nil {
		return err
	}
...
func (r *Backend) selectSystem(members []redfishLink) (*url.URL, error) {
	if len(members) == 0 {
...
func (r *Backend) reset(ctx context.Context, systemURL *url.URL, system redfishSystem, resetType string) error {
...
func (r *Backend) resolveLink(value string) (*url.URL, error) {
	return r.resolveLinkFrom(r.baseURL, value)
}

func (r *Backend) resolveLinkFrom(base *url.URL, value string) (*url.URL, error) {
...
func (r *Backend) request(ctx context.Context, method string, target *url.URL, body any, responseBody any) error {
