...
`CanUpdate*`ではSecret参照名の変更だけから安全性を推測しない。old/new双方のimmutable Secretを解決し、effective Talos configurationをrenderしてsemantic diff全体を分類する。missing、unreadable、generation不明は`unknown`としてpatchなしの`Failure`にする。Tart v1alpha1では自動replacementのopt-inを提供せず、unsafe diffはMachineを明示的に削除してRetained Hostを`Reprovision`する別lifecycleへ委譲する。

single-nodeのnode-disruptive updateは、`TartCluster.spec.updatePolicy.allowDowntime: true`が明示されている場合だけavailabilityを理由に永久blockせず、cordonとgraceful evictionを可能な範囲で試してpersistent data preservationを優先する。未指定または`false`なら安全停止する。multi-nodeではdrain成功を必須とする。
...
cluster secret bundleはClusterごとに一度だけという意味を「一つのSecretを永遠にimmutable」と解釈しない。`TartCluster.spec.id`を含むgeneration単位でimmutableなSecretを作成し、active generationを永続的な参照で切り替える。CA rotationはTalosの段階的なCA rotation operationへ委譲し、rotation開始前にTalosの準備結果から得た新しいsecret materialで次generationのimmutable Secretを`Pending`として永続化する。その後、accepted CA追加、issuing CA切替、certificate refresh、旧CA削除の正常完了を観測してから新generationをactiveに確定する。controller再起動時はPending bundleとTalosのobserved stateから再開し、rotation中は新しいMachineのprovisioningとAdoptを開始しない。Cluster存続中は過去generationをGCせず、削除時にDR保持方針を確認した後だけGCを許可する。
