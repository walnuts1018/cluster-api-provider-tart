...
Release workflowを再追加する前に、[検証方針](verification.md)の受け入れ確認を満たし、CIで生成、build、vet、lint、manifest検証、重要なGo testを再現できることを確認する。CAPI minorごとのunsafe diffに対するreplacement不発、Discovery、Cluster IDの復元・同名再作成分離、Talos CA rotationのPending generation永続化と完了後active切替、rotation対象外materialの保持、rollback、drain、availability理由に対する`TartCluster.spec.updatePolicy.allowDowntime`判定、storage payload保持をrelease gateへ含める。Workerの`OnDelete` strategyをin-place updateのtested profileへ含めない。
