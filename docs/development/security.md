...
Talos/Kubernetesのcluster-level PKIとsecret materialは`TartCluster.spec.id`を含むgeneration単位で生成する。Control Plane Providerがcontrol-plane MachineとBootstrap Secretより先にactive generationを作成し、Bootstrap Providerはread-onlyで参照する。BootstrapConfigごとにgenerateせず、Cluster namespaceの決定論的な`<cluster-name>-talos-secrets-<cluster-id>-g<generation>` Secretを正本として全Machineで共有する。各Secretはimmutableとし、CA rotationではgeneration Nを基にrotation対象のCAだけを更新した完全なgeneration N+1を`Pending` Secretとして先に永続化する。その後、Talos公式のaccepted CA追加、issuing CA切替、certificate refresh、旧CA削除のsemanticsをTalos machine configuration/APIでreconcileする。自動`rotate-ca`を完了後のmaterial回収として扱わず、Pending bundleとobserved stateから再開する。rotation対象外のetcd CA、service account keyなどは変更しない。正常完了を観測してから`TartCluster.status.activeSecretGeneration`へ新generationを反映する。rotation中は新しいMachineのprovisioningとAdoptを開始しない。Cluster存続中は過去generationをGCせず、Cluster削除時に全Managed Machineのshutdownとretention、バックアップ保持方針、Retained Hostの再利用制約を確認した後だけGCを許可する。必要なgenerationが欠落またはactive generationが不明な場合は自動再生成せず、`Ready=False`、`Reason=SecretBundleUnavailable`として報告する。Cluster削除後に残るRetained Hostはこのcredentialを使った`Adopt`を許可せず、`Reprovision`だけを許可する。
...
MHCやCAPI rolloutによるdelete-and-recreateも同じ保護境界に含める。Tartはlocal persistent stateの有無を判定しないため、初期運用ではMachineSetまたはControl PlaneのMachine templateへMachine生成前から`cluster.x-k8s.io/skip-remediation`を設定し、Machine作成後の後追いannotationだけに依存せず、MHCのreplacement remediationを既定で許可しない。force release、clean、reprovisioningは通常updateやdeleteの暗黙の副作用にしない。`TartHost`の直接削除も同様に、Claim中またはRetainedのHostは現在のbindingまたはretained recordに一致する`spec.forgetApproval`なしに削除せず、forget承認後もpower off、reset、disk wipeを行わない。
Tart v1alpha1では自動replacementやguided reprovisionのopt-inを提供しない。利用者のMachine削除はCAPIの通常replacement semanticsを発生させ得て、replacementが別のAvailable Hostをclaimする可能性がある。Retained Hostの`Reprovision`承認はdata破棄だけを許可し、Machine削除や同じHostへの再割り当てを開始しない。availability、PDB、capacityによるdrain失敗をdata preservationより優先して緩和する正本は`TartCluster.spec.updatePolicy.allowDowntime: true`であり、未指定または`false`ならavailability理由でも開始しない。destructive disk change、identity mismatch、Host mismatch、unsafe etcd membership change、quorum violationは緩和しない。
...
management clusterのDRでは、`TartHost.spec.id`、`TartCluster.spec.id`、stable hardware identity、consumerRef、retainedFrom、CAPI Machine/provider resource、全secret bundle generation、power/boot設定を同じ整合点からバックアップする。復元時は`tart.cluster.x-k8s.io/restore-approved: "true"` annotationとinfra administratorの権限境界を要求し、既存の`spec.id`を保持する。通常CREATEでpresetされた`spec.id`は受け入れず、同名Clusterの再作成では新しいCluster IDを要求する。復元後はClusterやHostのobject metadata UID変更を永続identityの変更と解釈せず、関係するResource、Cluster ID、active bundle generationを観測してから副作用を再開する。古いbundleやRetained Hostを再関連付けしない。`clusterctl move`でこの復元契約を代用せず、bundleが欠落または世代不明なら`Adopt`せず`Reprovision`または明示的な管理者復旧を要求する。
