...
MHCやCAPI rolloutによるdelete-and-recreateも同じ保護境界に含める。Tartはlocal persistent stateの有無を判定しないため、初期運用ではMachineSetまたはControl PlaneのMachine templateへMachine生成前から`cluster.x-k8s.io/skip-remediation`を設定し、Machine作成後の後追いannotationだけに依存せず、MHCのreplacement remediationを既定で許可しない。force release、clean、reprovisioningは通常updateやdeleteの暗黙の副作用にしない。`TartHost`の直接削除も同様に、Claim中またはRetainedのHostは`tart.cluster.x-k8s.io/forget-approved: "true"` annotationなしに削除せず、forget承認後もpower off、reset、disk wipeを行わない。
