...
- disk selection、partition作成、slot/verity書き込み、検証は全hardwareで共通Provisioning Agentが行う。
- WoL機ではiPXE、BMC搭載機ではRedfish PXE、HTTP boot、Virtual MediaのいずれかをAgentのboot transportとして選ぶ。
- Redfish Virtual Mediaを優先できる機器ではTFTP依存を除き、boot mode、Secure Boot、one-time bootをBMC driverで設定する。
