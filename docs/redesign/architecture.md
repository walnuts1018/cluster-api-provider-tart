...
`spec.consumerRef`はcontrollerがresourceVersion付きUpdateで設定する割当である。CapabilityはDriver discoveryの結果として`status.capabilities`へ保存する。Secret値はCRへ埋め込まず、Secret参照だけをSpecへ保存する。
...
| `Retained` | Data保持、State消去済み | `Cleaning` |
| `Detached` | State/Dataを保持して管理対象から外した状態 | `Cleaning`、`RecoveryRequired` |
...
terminal Phaseは`Succeeded`、`Failed`、`RecoveryRequired`の3つとする。`Error`から復帰するHostは`status.lastStablePhase`へ戻す。`lastStablePhase`へ保存できる値は`Available`、`Provisioned`、`Retained`、`Detached`だけとする。
...
| `Temporary` | 合計3回試行。1回目は即時、2回目は1秒後、3回目は2秒後 |
...
State/DataはPlanに記録したGPT PARTUUIDで識別し、OS image内の明示的な`.mount` unitと依存関係により、kubelet/container runtimeより前にmountする。`/dev/sda5`のような列挙順序へ依存しない。必須mountへ`nofail`を付けず、失敗時にOS側の空directoryへ新しいNode identityを書かせない。
...
ManifestはRFC 8785 JSON Canonicalization Schemeでserializeし、Manifest自体を署名対象とする。成果物はCIでSBOM、provenance、署名を生成する。Agentはdownload後かつblock deviceへ書く前にManifest署名とpayload digestを検証し、書き込み後に対象block deviceのSHA-256とdm-verity root hashを検証する。

### Build toolchain

Kubernetes SIGs Image BuilderはCAPI向けVM/whole-disk raw imageとKubernetes設定の生成に利用できる。2026-06-28時点のraw provider文書で確認できるUbuntu targetは24.04であり、26.04 raw targetの欠落はopen Issueとして追跡されている。Image Builderのraw成果物はA/B filesystem slot、dm-verity、State/Data契約、Debian 13対応をそのまま提供しない。

Task 05で、Image Builder rawの変換、Ansible roleだけの再利用、mkosi + systemd-repartの独自pipelineを比較する。検証前にImage Builder採用または独自実装へ固定しない。
...
Bundleは`apiVersion`、`format`、`payload`、`payloadDigest`、`machineUID`、`operationUID`を持つ。MVPは`format=cloud-config`だけを受理する。OS image内のAdapterはState/Data bind mount成立後に一度だけ適用する。成功markerはpayload digest、適用時刻、Adapter versionを含むJSON fileとしてStateと同じfilesystem上の一時fileへ書き、fsync後にrenameする。
...
1. `CanUpdateMachine`/`CanUpdateMachineSet`はcurrentとdesiredのMachine、InfraMachine、BootstrapConfigをRFC 8785 Canonical JSONへ変換して差分fieldを列挙する。
2. OSOnlyで許可する差分はOS Artifact参照とUpdate Policyだけとする。Machine versionまたはBootstrap payload digestが変わる場合はOSOnlyとして扱わない。
3. Extensionが返す Machine、BootstrapConfig、InfraMachine に対する JSON Patch、または CAPI が作成する patch で全差分を覆えない場合はin-placeを拒否する。CAPIは通常置換を選択する。
...
- 応答結果が不明な通信切断後に同じtokenを再利用せず、AgentはInitial Credentialを用いて `/v1/agent/register` から再開し、controllerはこれを受けて新しいsession tokenを発行する。
- 一時OSの時計が信頼できない環境を考慮し、private CA/SPKI pinningと信頼できる時刻取得の方式をplatform profileで定める。
- `TartCluster.spec.artifactPolicy.allowedRegistries`を必須かつ1件以上とし、OCI参照のhostname/portが完全一致するRegistryだけを許可する。suffix一致やwildcardを使用しない。
