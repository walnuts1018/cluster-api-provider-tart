...
3. Task 01のboot trial rollbackについては、CIでsimulator rollback evidence、boot metadata 永続化証跡、boot選択実証証跡を継続収集し、rollback判定に使った入力、期待した状態遷移、Artifact/Profile識別子をartifactへ保存する。これは bootloader 実機/QEMU 実証の代替ではなく、役割分担を分けて残差を減らすための継続証跡とする。
...
6. acceptance 8 の CI 証跡要件は、次の3系統で管理する。
