...
5. control planeではsnapshot取得後に`kubeadm upgrade apply`相当を実行している。
...
Ubuntu 26.04成果物はx86-64-v1でビルドし、Intel Sandy Bridge相当の実機または同等CPU modelのQEMUでboot testを行う。Ubuntu cloud imageの既定CPU levelを本Providerの対応根拠として使用しない。
...
|---|---|---|---|---|
| `WipeAll` | 消去 | 消去 | `Available` | 許可 |
| `RetainData` | 消去 | 保持・隔離 | `Retained` | 禁止 |
| `RetainState` | 保持 | 保持 | `Detached` | 禁止 |

`RetainData`のHostは、利用者が保持Dataを明示的にadoptするか`WipeAll`を実行するまで`Available`へ戻さない。

delete-first host reuseで新しいMachineを割り当てる場合は`WipeAll`、またはState消去完了を証明する専用Clean Operationを必須とする。旧providerID、node credential、Bootstrap成功markerを新Machineへ引き継がない。

## 8. 排他性と再実行

- 1つの`TartHost`が参照できるactive `TartMachine`は最大1つ。
- 1つの`TartHost`で`Pending`から`RecoveryRequired`までの非terminal Operationは最大1つ。
