...
| 7 | 実装済み、OS image統合未検証 | Provisioning Agentに`--apply-bootstrap-only`を追加し、Bundleを一時fileへ保存、local cloud-config adapter成功後にpayload原本を削除し、payload digest/adapter version/適用時刻だけをState markerへ残す処理を実装。first-boot経由のOS image統合と実機確認は未検証 |
...
- `docs/redesign/runbooks/07-initial-provisioning-simulated-record.md` に、Bootstrap payload削除、単回Session、`AwaitingHealth`維持、Cleaning phase遷移の疑似証跡を追加した
