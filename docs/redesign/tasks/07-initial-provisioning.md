...
13. `machineRef` が空の `WipeAll` タイプの `TartHostOperation` を手動作成したとき、対象ホストの Wipe 処理が走り `Available` に遷移する。
14. `WipeAll` の Operation 実行時、ディスク容量に応じた適切な deadline (タイムアウト) が Plan に設定される。
