...
func TestWorkflowは署名済みPlanだけを実行する(t *testing.T) {
	...
	outcome := workflow.Do(
		t.Context(),
		Command{Plan: SignedPlan{Plan: plan, Signature: signature}, Step: domain.StepPreflightCompleted},
	)
	event, present := outcome.Value().Value()
	if !present {
		t.Fatalf("RunSignedStep() = %#v, want success", outcome)
	}
	if event.(SignedStepRun).Result != (distribution.StepResult{}) {
		t.Fatalf("RunSignedStep() result = %#v, want empty", event)
	}
...
func TestWorkflowはSnapshotRefなしにDistributionAppliedを実行しない(t *testing.T) {
	...
	if outcome := workflow.Do(
		t.Context(),
		Command{Plan: SignedPlan{Plan: plan, Signature: signature}, Step: domain.StepDistributionApplied},
	); !outcome.IsFailure() {
