...
func TestWorkflowはLifecycleStepをDriverの型付き操作へDispatchする(t *testing.T) {
	...
	for _, step := range []domain.Step{
		...
		if outcome := workflow.Do(t.Context(), Command{Plan: plan, Step: step}); outcome.IsFailure() {
			t.Fatalf("RunStep(%q) = %#v, want success", step, outcome)
		}
	}
...
func TestWorkflowはRestoreTestに失敗したSnapshotを拒否する(t *testing.T) {
	...
	outcome := workflow.Do(t.Context(), Command{Plan: controlPlanePlan(t), Step: domain.StepSnapshotCreated})
	if !outcome.IsFailure() {
		t.Fatalf("RunStep(SnapshotCreated) = %#v, want restore test error", outcome)
	}
}
...
func TestWorkflowはSnapshotRefなしにKubeadmApplyを実行しない(t *testing.T) {
	...
	if outcome := workflow.Do(t.Context(), Command{Plan: plan, Step: domain.StepDistributionApplied}); !outcome.IsFailure() {
