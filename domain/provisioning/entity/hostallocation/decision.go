...
func Decide(command Command) Result {
	...
	if len(unsupported) > 0 {
		return sharedresult.Failure[Allocated, Failure](UnsupportedCapability{Missing: unsupported})
	}
	return sharedresult.Failure[Allocated, Failure](NoMatchingHost{Requirements: command.Requirements})
}
