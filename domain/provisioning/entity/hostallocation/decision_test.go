...
func TestDecideAllocatesFirstMatchingCandidate(t *testing.T) {
	...
	allocated, present := result.Value().Value()
	if !present {
		t.Fatal("Decide() returned failure, want Allocated")
	}
...
func TestDecideReturnsUnsupportedCapabilityWhenOnlyCapabilityBlocksAllocation(t *testing.T) {
	...
	failureValue, present := result.FailureValue().Value()
	if !present {
		t.Fatal("Decide() returned success, want UnsupportedCapability")
	}
	failure, ok := failureValue.(UnsupportedCapability)
	if !ok {
		t.Fatalf("failure = %T, want UnsupportedCapability", failureValue)
	}
...
func TestDecideReturnsNoMatchingHostWhenCandidateDimensionsDoNotMatch(t *testing.T) {
	...
	failure, present := result.FailureValue().Value()
	if !present {
		t.Fatal("Decide() returned success, want NoMatchingHost")
	}
	if _, ok := failure.(NoMatchingHost); !ok {
		t.Fatalf("failure = %T, want NoMatchingHost", failure)
	}
}
...
func TestDecideKeepsClaimedHostForSameMachine(t *testing.T) {
	...
	if _, present := Decide(command).Value().Value(); !present {
		t.Fatal("Decide() returned failure, want Allocated")
	}
}
