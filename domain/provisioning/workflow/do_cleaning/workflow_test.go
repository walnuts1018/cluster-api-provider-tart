...
func TestWorkflowはOperation作成後に一致する署名済みCleaningPlanを保存する(t *testing.T) {
	...
	outcome := workflow.Do(t.Context(), Command{Machine: machine, Host: host})
	event, present := outcome.Value().Value()
	if !present {
		t.Fatalf("StartCleaning() = %#v, want success", outcome)
	}
	started := event.Operation
...
func TestWorkflowは再試行時に保存済みDeadlineから同じCleaningPlanを再生成する(t *testing.T) {
	...
	firstOutcome := workflow.Do(t.Context(), Command{Machine: machine, Host: host})
	firstEvent, present := firstOutcome.Value().Value()
	if !present {
		t.Fatalf("first StartCleaning() = %#v, want success", firstOutcome)
	}
	first := firstEvent.Operation
	firstDigest := writer.planDigest(t)

	workflow.now = func() time.Time { return time.Date(2026, 7, 12, 11, 0, 0, 0, time.UTC) }
	secondOutcome := workflow.Do(t.Context(), Command{Machine: machine, Host: host})
	secondEvent, present := secondOutcome.Value().Value()
	if !present {
		t.Fatalf("second StartCleaning() = %#v, want success", secondOutcome)
	}
	second := secondEvent.Operation
