...
func TestWorkflowReturnsAllocationPending(t *testing.T) {
	...
	pendingEvent, ok := result.(HostAllocationPending)
	if !ok {
		t.Fatalf("Start() result = %T, want AllocationPending", result)
	}
	pending := pendingEvent.Result
...
func TestWorkflowPersistsProvisionPlanAfterOperationStart(t *testing.T) {
	...
	startedEvent, ok := result.(MachineProvisioningStarted)
	if !ok {
		t.Fatalf("Start() result = %T, want Started", result)
	}
	started := startedEvent.Result
	if writer.calls != 1 {
...
func TestWorkflowAcceptsAPIServerDeadlinePrecisionOnOperationStart(t *testing.T) {
	...
	if _, ok := result.(MachineProvisioningStarted); !ok {
		t.Fatalf("Start() result = %T, want Started", result)
	}
	if writer.calls != 1 {
		t.Fatalf("PlanWriter calls = %d, want 1", writer.calls)
