...
func BuildUpdatePlan(
	...
	activeValue := string(input.TartMachine.Status.ActiveSlot)
	active, present := slotdomain.Parse(activeValue).Value().Value()
	if !present {
		return SignedUpdatePlan{}, fmt.Errorf("parse active slot: unsupported value %q", activeValue)
	}
	target, present := active.Inactive().Value().Value()
	if !present {
		return SignedUpdatePlan{}, fmt.Errorf("select inactive slot for %q", active)
	}
