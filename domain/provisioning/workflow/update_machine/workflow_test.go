...
import (
	...
	nodelifecycleapp "github.com/walnuts1018/cluster-api-provider-tart/domain/node/workflow/run_signed_step"
...
func mustStart(t *testing.T, workflow *Workflow, command Command) StartResult {
	t.Helper()
	outcome := workflow.Do(t.Context(), command)
	event, present := outcome.Value().Value()
	if !present {
		t.Fatalf("Start() = %#v, want success", outcome)
	}
	return event.Result
}
...
func TestWorkflowはOperation作成後に一致する署名済みPlanを保存する(t *testing.T) {
	...
	workflow := NewWorkflow(starter, writer, testPlanSigner(t), nil, PlanSigner{})

	result := mustStart(t, workflow, input)
...
func TestWorkflowはKubernetesBinary更新でNodeLifecyclePlanも保存する(t *testing.T) {
	...
	workflow := NewWorkflow(starter, agentWriter, testPlanSigner(t), nodeWriter, testPlanSigner(t))
...
func workflowInput(t *testing.T) Command {
	t.Helper()
	input := updateInput()
	input.PlanDigest = ""
