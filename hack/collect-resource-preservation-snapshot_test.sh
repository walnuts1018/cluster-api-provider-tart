...
cp "$script_dir/testdata/fake-kubectl" "$work_dir/bin/kubectl"
chmod +x "$work_dir/bin/kubectl"

PATH="$work_dir/bin:$PATH" \
  "$script_dir/collect-resource-preservation-snapshot.sh" lifecycle-e2e "$work_dir/snapshot.json"
...
"$script_dir/verify-resource-preservation.sh" "$work_dir/snapshot.json" "$work_dir/snapshot.json"
