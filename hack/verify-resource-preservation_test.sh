...
expect_failure() {
  ...
  if "$script_dir/verify-resource-preservation.sh" "$before" "$after" >/dev/null 2>&1; then
...
"$script_dir/verify-resource-preservation.sh" "$work_dir/before.json" "$work_dir/same.json"
