...
import (
	...
	inplaceupdatedomain "github.com/walnuts1018/cluster-api-provider-tart/domain/provisioning/entity/inplaceupdate"
	appupdate "github.com/walnuts1018/cluster-api-provider-tart/domain/provisioning/workflow/update_machine"
	bootreportdomain "github.com/walnuts1018/cluster-api-provider-tart/domain/shared/bootreport"
...
func applyBootFailureAttempt(
	...
	activeSlot, present := slotdomain.Parse(expected.RollbackSlot).Value().Value()
	if !present {
		return fmt.Errorf("parse rollback slot: unsupported value %q", expected.RollbackSlot)
	}
	targetSlot, present := slotdomain.Parse(expected.ActiveSlot).Value().Value()
	if !present {
		return fmt.Errorf("parse active slot: unsupported value %q", expected.ActiveSlot)
	}
