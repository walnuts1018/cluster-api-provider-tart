...
type recordingWorkflow struct {
	input     application.Command
	operation *infrastructurev1beta1.TartHostOperation
}

func (workflow *recordingWorkflow) Do(
	_ context.Context,
	input application.Command,
) sharedresult.Result[application.Event, sharedworkflow.Failure] {
	workflow.input = input
	return sharedworkflow.Succeeded[application.Event](application.Event{Result: application.StartResult{Operation: workflow.operation}})
}
