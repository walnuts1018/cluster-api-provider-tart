...
const (
	PlanSecretSuffix       = "-node-lifecycle-plan"
	PlanSecretPlanKey      = "plan.json"
	PlanSecretSignatureKey = "signature.json"

	planWriterFieldManager = "tart-node-lifecycle-plan"
)

// PlanWriterは署名済みNode Lifecycle PlanをOperation所有のimmutable Secretへ保存する。
...
func (writer *PlanWriter) Write(
	...
	plan application.ValidatedPlan,
	signature agentprotocol.Signature,
) error {
	if operation == nil || operation.Name == "" || operation.UID == "" {
		return fmt.Errorf("persist Node Lifecycle Plan: TartHostOperation name and UID are required")
	}
	if plan.Value().OperationID != operation.Spec.OperationID {
		return fmt.Errorf("persist Node Lifecycle Plan: operation ID does not match")
	}
	...
	if err != nil {
		return fmt.Errorf("persist Node Lifecycle Plan: canonicalize Plan: %w", err)
	}
	signatureJSON, err := json.Marshal(signature)
	if err != nil {
		return fmt.Errorf("persist Node Lifecycle Plan: encode signature: %w", err)
	}
	...
	if err := writer.client.Get(ctx, key, current); err == nil {
		...
		return fmt.Errorf("get Node Lifecycle Plan Secret: %w", err)
	}
	...
	if err := writer.client.Apply(
		...
		if apierrors.IsAlreadyExists(err) || apierrors.IsConflict(err) {
			if getErr := writer.client.Get(ctx, key, current); getErr != nil {
				return fmt.Errorf("get competing Node Lifecycle Plan Secret: %w", getErr)
			}
			return validateExistingPlanSecret(current, operation, data)
		}
		return fmt.Errorf("apply Node Lifecycle Plan Secret: %w", err)
	}
	return nil
}
...
func validateExistingPlanSecret(
	...
	if secret.Immutable == nil || !*secret.Immutable {
		return fmt.Errorf("existing Node Lifecycle Plan Secret is mutable")
	}
	if !apiequality.Semantic.DeepEqual(secret.Data, expectedData) {
		return fmt.Errorf("existing Node Lifecycle Plan Secret content conflicts with TartHostOperation")
	}
	...
	return fmt.Errorf("existing Node Lifecycle Plan Secret is not owned by TartHostOperation")
}
