...
func (writer *StatusWriter) HandleBootTrialDeadline(
	...
	targetSlotValue := string(operation.Spec.TargetSlot)
	targetSlot, present := slotdomain.Parse(targetSlotValue).Value().Value()
	if !present {
		return fmt.Errorf("parse target slot: unsupported value %q", targetSlotValue)
	}
	activeSlot, present := targetSlot.Inactive().Value().Value()
	if !present {
		return fmt.Errorf("select active slot for target %q", targetSlot)
	}
	return retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		current := &infrastructurev1beta1.TartHostOperation{}
		if err := writer.client.Get(ctx, client.ObjectKeyFromObject(operation), current); err != nil {
