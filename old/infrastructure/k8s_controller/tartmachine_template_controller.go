...
func (r *TartMachineTemplateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if _, err := workflowresult.Unwrap(r.lifecycleWorkflow().Do(ctx, machinetemplatelifecycle.Command{Template: &template})); err != nil {
		return ctrl.Result{}, err
	}
...
func (r *TartMachineTemplateReconciler) lifecycleWorkflow() *machinetemplatelifecycle.Workflow {
	...
	return machinetemplatelifecycle.NewWorkflowWithFinalizer(resourcefinalizer.NewTartMachineTemplateService(r.Client))
}
