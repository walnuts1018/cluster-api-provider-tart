...
func externalContractSimulatorBinary(t *testing.T) string {
	...
	externalContractBuildOnce.Do(func() {
		...
		command := exec.CommandContext(t.Context(), "go", "build", "-o", externalContractBuildPath, "./cmd/redfish-simulator")
		command.Dir = externalContractRepoRoot(t)
		command.Env = append(
			os.Environ(),
		...
		if err != nil {
			externalContractBuildErr = fmt.Errorf("go build ./cmd/redfish-simulator: %w\n%s", err, output)
		}
	})
