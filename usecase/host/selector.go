...
// MatchesForFailureDomainはHostがhostSelectorとCAPI MachineのFailure Domainを満たすかを返す。Failure DomainはCAPI Machineが所有するため、HostSelectorへ複製しない。selectorの一致とallocation eligibilityは独立した観測であるため、Eligibilityによる絞り込みは呼び出し側が別途行う。
